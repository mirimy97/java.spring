...
11. [추상클래스(abstract class)](#11-추상클래스abstract-class)

    - [추상클래스란?](#-추상클래스란)

    - [추상메서드(abstract method)](#-추상메서드abstract-method)
...
- 추상클래스 = 미완성 설계도 = 미완성 메서드(추상 메서드)를 포함
- 추상클래스로는 인스턴스를 생성할 수 없다.
- 새로운 클래스의 바탕이 되는 조상 클래스로서 의미를 갖는다.
...
- 추상 메서드를 포함하지 않더라도 `abstract` 제어자가 붙어있으면 인스턴스를 생성할 수 없다.

## 💡 추상메서드(abstract method)

- 미완성 메서드
- 선언부 O 구현부 X
- 조상 클래스에서 선언부와 주석(설명)만 작성해놓고, 자손 클래스에서 구현부(내용)을 작성한다.

```java
/* 주석을 통해 어떤 기능을 수행하는 메서드인지 작성한다, */
abstract 리턴타입 메서드이름();
```

- 자손클래스가 조상클래스의 추상메서드를 모두 구현하지 않았다면, 자손클래스도 추상클래스로 지정 (`abstract`) 해주어야 한다.
...
1. 여러 클래스에 공통으로 사용될 수 있는 클래스를 바로 작성하기
2. 기존 클래스의 공통적인 부분을 뽑아서 추상클래스로 만들기
   1. 자손 클래스들에 공통적인 기능을 하는 메서드는 그냥 조상클래스에서 작성한다.
   2. 공통적으로 포함되어야 하나, 서로 다른 기능을 하는 경우에는 조상클래스에 추상메서드(`abstract`)로 선언부만 작성한다 → **자손클래스에서 구현을 강제**할 수 있다.
