...
12. [인터페이스(interface)](#12-인터페이스interface)
...
- 추상클래스와 비슷하지만 추상화 정도가 높다.
- 추상클래스가 ‘미완성 설계도’라면, 인터페이스는 밑그림만 있는 ‘기본 설계도’
- 가질 수 있는 멤버 : 일반메서드 X, 멤버변수 X, 추상메서드 O, 상수 O
...
- 클래스를 작성하는 것과 같지만, `class` 대신 `interface` 키워드를 사용한다.
- 클래스와 같이 접근제어자로 `public` 또는 `(default)`를 사용할 수 있다.

### 제약사항 (편의상 생략하는 경우가 많음)

- 모든 멤버변수는 `public static final`이어야 하며, 이를 생략할 수 있다.
- 모든 메서드는 `public abstract`(추상메서드)이어야 하며, 이를 생략할 수 있다.
  - (단, JDK1.8부터는 `static` 메서드와 `default` 메서드는 예외)

## 💡 인터페이스의 상속

- 인터페이스로부터만 상속받을 수 있다.
- 클래스와 달리 **다중상속이 가능**하다. (~~하지만 실제로는 잘 사용하지 않는다.~~)
...
- 인터페이스를 구현한다는 의미의 키워드 `implements`를 사용한다.

```java
interface 인터페이스이름 {
		void 메서드이름(매개변수);    // (public abstract) void 생략
}

class 클래스이름 implements 인터페이스이름 {
		// 인터페이스에 정의된 추상메서드를 구현해야 한다.
    // 오버라이딩할 때는 조상의 메서드보다 넓은 범위의 접근제어자를 지정해야 한다. (public)
		public void 메서드이름(매개변수) { /* 실제 구현 내용 */ }
}
```
...
### [🔗 인터페이스를 이용한 다형성 실습 예제](https://github.com/mirimy97/java.spring/blob/master/javajungsuk/ch07/ParserTest.java)

- 상속과 마찬가지로, 인터페이스를 조상으로 볼 수 있다.
  - 인터페이스 타입으로 구현한 클래스를 참조할 수 있다.
    ```java
    Fightable f = (Fightable) new Fighter();
      // 또는
    Fightable f = new Fighter();    // 자손(구현체) -> 조상(인터페이스) 형변환 생략 가능
    ```
  - 구현한 클래스 → 인터페이스 타입으로의 형변환도 가능하다.
    - 구현한 클래스의 매개변수의 타입으로 인터페이스 타입이 사용될 수 있다.
    - **리턴타입으로 인스턴스의 타입을 지정할 수 있다. (이 경우, 해당 인터페이스를 구현한 클래스의 인스턴스를 반환하는 것을 의미한다.)**

## 💡 인터페이스의 장점

1. **개발시간의 단축**
   1. 일단 인터페이스가 작성되면 이를 사용하는 프로그램을 작성할 수 있다.
   2. 한 쪽에서는 구현하는 클래스를 작성하고, 다른 한 쪽에서는 구현 내용을 몰라도 메서드 선언부만으로 프로그램을 작성할 수 있기 때문이다.
2. **표준화 가능**
   1. 기본틀을 인터페이스로 작성함으로써 일관되고 정형화된 프로그램 개발이 가능
3. **서로 관계없는 클래스들에게 관계를 맺어 줄 수 있다.**
   1. 상속관계가 결정된 상태애서, 1. 서로 상속관계가 아니고, 2. 같은 조상클래스를 가지지도 않은 클래스들 / 3. 혹은 같은 조상클래스를 갖지만 그 중 일부에 하나의 인터페이스를 구현하도록 함으로써 관계를 맺어줄 수 있다.
   2. [🔗 실습 예제](https://github.com/mirimy97/java.spring/blob/master/javajungsuk/ch07/RepairableTest.java)
4. **독립적인 프로그래밍 가능**
   1. 클래스의 선언과 구현을 분리함으로써 클래스-클래스 간의 직접적 관계를 인터페이스를 이용한 간접적 관계로 변경할 수 있다.
   2. 한 클래스의 변경이 관련된 다른 클래스에 영향을 미치지 않는 독립적 프로그래밍이 가능하다.

## 💡 인터페이스의 의의

- 인터페이스의 의의는 메서드의 사용자(A 클래스)와 사용자(B클래스)가 서로에 덜 의존하게 된다는 것에 있다.
- 예를들ㅇ
- 하지만 B의 **선언부와 구현부를 분리**하여 선언부인 인터페이스를 구현하게 한다면,
- A에서는 `I i`, `i.methodB`와 같이 인터페이스만 사용하면 된다.

## 💡 디폴트 메서드와 static 메서드

- 원래는 인터페이스에 추상메서드(`public abstract`)만 선언할 수 있으나,
  **JDK1.8부터** 디폴트 메서드와 static 메서드도 추가할 수 있게 됐다.

### static 메서드

- 인스턴스와 관계없는 독립 메서드이기 때문에 인터페이스에 추가하지 못할 이유가 없었지만, Java를 보다 쉽게 배울 수 있도록 규칙을 단순화하기 위해 제외됐었다.
- 인터페이스의 static메서드 역시 접근제어자가 항상 `public`이며, 생략가능하다.
...
- 추상메서드 X 기존의 **조상 클래스에 새로운 메서드를 추가한 것과 동일**해진다.
- 인터페이스에 새로운 메서드를 추가하는 경우, 인터페이스를 구현한 모든 클래스에 그 기능을 구현해야 하는 문제를 해결하기 위해 고안되었다.
- **기존 메서드와 이름이 중복되는 경우**에는
  - 여러 인터페이스의 default 메서드 간의 충돌 : 인터페이스 구현 클래스에서 default 메서드를 오버라이딩해야 한다.
  - default 메서드와 조상 클래스 메서드 간의 충돌 : 조상 클래스 메서드가 우선
  - 규칙이 복잡하다면 그냥 구현 클래스에서 오버라이딩하자.
- 접근제어자가 `public`이며, 생략가능하다.
